<div id="remove-attribute-modal-{{$id}}" class="fixed z-10 inset-0 overflow-y-auto hidden">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 transition-opacity" onclick="removeAttributeModal({{ $id }})">
        ...
        <div class="bg-white inline-block align-bottom rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            ...
            {{-- modal body --}}
            <form action="{{ route('attribute.destroy',$id) }}" method="POST">
                @csrf
                @method("DELETE")
                ...
                <div class="bg-gray-100 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    ...
                    <button type="button"
                            ...
                            onclick="removeAttributeModal({{ $id }})">
                        Cancel
                    </button>
                </div>
            </form>
...
<script>
    function removeAttributeModal($id) {
        const modal = document.getElementById("remove-attribute-modal-" + $id)
